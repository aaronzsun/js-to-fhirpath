...
describe('#fhirconvert()', function() {
    ...
    context('Functions', function() {
        ...
        it('CEILING Exp', function() {
          expect(fhir.fhirconvert("CEILING(a+b*3)", vars)).to.equal("((%a+%b*3).ceiling())");
        })
        it('FLOOR', function() {
            expect(fhir.fhirconvert("FLOOR(a)", vars)).to.equal("((%a).floor())");
        })
        it('FLOOR Exp', function() {
            expect(fhir.fhirconvert("FLOOR(a+b*3)", vars)).to.equal("((%a+%b*3).floor())");
        })
        it('ABS', function() {
            expect(fhir.fhirconvert("ABS(a)", vars)).to.equal("((%a).abs())");
        })
        it('ABS Exp', function() {
            expect(fhir.fhirconvert("ABS(a+b*3)", vars)).to.equal("((%a+%b*3).abs())");
        })
        it('SQRT', function() {
            expect(fhir.fhirconvert("SQRT(a)", vars)).to.equal("((%a).sqrt())");
        })
        it('SQRT Exp', function() {
            expect(fhir.fhirconvert("SQRT(a+b*3)", vars)).to.equal("((%a+%b*3).sqrt())");
        })
        it('TRUNCATE', function() {
            expect(fhir.fhirconvert("TRUNCATE(a)", vars)).to.equal("((%a).truncate())");
        })
        it('TRUNCATE Exp', function() {
            expect(fhir.fhirconvert("TRUNCATE(a+b*3)", vars)).to.equal("((%a+%b*3).truncate())");
        })
        it('EXP', function() {
            expect(fhir.fhirconvert("EXP(a)", vars)).to.equal("((%a).exp())");
        })
        it('EXP Exp', function() {
            expect(fhir.fhirconvert("EXP(a+b*3)", vars)).to.equal("((%a+%b*3).exp())");
        })
        it('LN', function() {
        ...
        it('LOG Exp', function() {
            expect(fhir.fhirconvert("LOG(a+c, a+b*3)", vars)).to.equal("%a+%b*3.log(%a+%c)");
        })
    })

    context('Nested Functions', function() {
        it('Nested 1', function() {
          expect(fhir.fhirconvert("CEILING(FLOOR(ABS(a+b*3)))", vars)).to.equal("((((((%a+%b*3).abs())).floor())).ceiling())");
        })
        it('Nested 2', function() {
            expect(fhir.fhirconvert("CEILING(LOG(2, ABS(a+b*3)))", vars)).to.equal("((((%a+%b*3).abs().log(2))).ceiling())");
        })
    })
